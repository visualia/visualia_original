...
Project public components, available in the Markdown templates and their index file.
...
Internal components and utility function and their index file.

#### /src/utils + /src/utils.js

Public utility functions that are available in the Markdown templates and their index file
...
Rollup config files for building external dependencies. They are used in `npm run build:deps` and in `npm run build:lib` build scripts.
...
Visualia relies on a suite of unit tests what verify framework's utility and internal functions work correctly.
